...
namespace SourceEngine
{
	...
	void ConVar::RemoveChangeCallback(FnChangeCallback_t callbackToRemove)
	{
		if (callbackToRemove) {
			if (m_fnChangeCallbacks.Find(callbackToRemove) != -1) {
				for (auto c = 0; c < m_fnChangeCallbacks.Count(); c++)
				{
					if (m_fnChangeCallbacks[c] == callbackToRemove)
					{
						m_fnChangeCallbacks[c] = (FnChangeCallback_t)NULL;
					}
				}
			}
			else {
